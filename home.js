...
const canWeDeliverTwo = (zipCode2) => {
    for (let i = 0; i < deliveryAreaZipCodes.length; i++){
        if (zipCode2.includes(deliveryAreaZipCodes)){
        return `You're in our delivery zone!`
         } else { return `Sorry, we can't deliver to that address`
}
}
...
//CODE HERE
function changeDiscount(param1) {
    param1 = param1.title.replace('15', '10')
    deals.title = param1
    return deals
}
changeDiscount(deals)
console.log(deals)
...
//CODE HERE
function extend(param1) {
    param1 = deals.desc.replace('March', 'April')
    deals.title = param1
    return deals
}
extend(deals)
console.log(deals.desc.trim())